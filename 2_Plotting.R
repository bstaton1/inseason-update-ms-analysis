...
# resolution of figures (pixels per inch)
ppi = 600
...
text(x = usr[2] - xdiff * 0.41, y = usr[4] - ydiff * 0.1,
     labels = TeX("$\\Mean(\\epsilon_{F,t}) = 0.008$"), pos = 4)
text(x = usr[2] - xdiff * 0.41, y = usr[4] - ydiff * 0.2,
     labels = TeX("$\\SD(\\epsilon_{F,t}) = 0.279$"), pos = 4)
box()
dev.off()

#### FIGURE 2: RUN TIMING OBSERVATIONS VS. FORECASTS ####
# load data
btf = read.table(file.path(data_dir, "BTF_Data.txt"), stringsAsFactors = F, header = T)
true_N = read.table(file.path(data_dir, "Total Run_Data.txt"), header = T)
...
jpeg(file.path(out_dir, "Figure2.jpg"), h = 3 * ppi, w = 3.4 * ppi, res = ppi)
...
jpeg(file.path(out_dir, "Figure3.jpg"), h = 5 * ppi, w = 3 * ppi, res = ppi)
...
prior_summ = readRDS(file = file.path(out_dir, "prior_summ.rds"))
like_summ = readRDS(file = file.path(out_dir, "like_summ.rds"))
post_summ = readRDS(file = file.path(out_dir, "post_summ.rds"))

# the dimensions of each of these arrays is [date,summary.stat,year,model]
...
pt.cex = 1.3
jpeg(file.path(out_dir, "Figure4.jpg"), h = 5 * ppi, w = 6 * ppi, res = ppi)
...
ppi = 600
jpeg(file.path(out_dir, "Figure5.jpg"), h = 9.3 * ppi, w = 7.2 * ppi, res = ppi)
...
write.csv(coverage_out, file.path(out_dir, "coverage_table.csv"))
