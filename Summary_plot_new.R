...
m1 = 3; m2 = 5
ppi = 600
jpeg("Summary_new_no_harv.jpg", h = 5 * ppi, w = 6 * ppi, res = ppi)
par(mfrow = c(2,2), mar = c(1,3.1,1,0.5), yaxs = "i", oma = c(4,0,0,0))
plot(like_mae[,m1], type = "o", pch = 17, ylim = c(0, 0.4), axes = F, ann = F)
...
plot(like_mpe[,m1], type = "o", pch = 17, ylim = c(-0.1, 0.1), axes = F, ann = F)
usr = par("usr"); xdiff = diff(usr[1:2]); ydiff = diff(usr[3:4])
...
plot(like_sig[,m1], type = "o", pch = 17, ylim = c(0, 0.4), axes = F, ann = F)
usr = par("usr"); xdiff = diff(usr[1:2]); ydiff = diff(usr[3:4])
text(x = usr[2] + xdiff * 0.02, y = usr[4] - ydiff * 0.05, labels = "(c)", font = 2, pos = 2, cex = 1.1)
text(x = usr[2] + xdiff * 0.02, y = usr[4] - ydiff * 0.15, labels = "sd(log(est/true))", font = 2, pos = 2, cex = 1.1)
